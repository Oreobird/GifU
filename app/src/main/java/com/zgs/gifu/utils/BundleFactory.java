...
public class BundleFactory {
    ...
    public static Bundle createTransitionBundle(Context context, View fromView,
                                                int position, List<ImageObj> imageObjList) {

        int[] screenLocation = new int[2];
        fromView.getLocationOnScreen(screenLocation);

        final TransitionData transitionData = new TransitionData(context, screenLocation[0],
                screenLocation[1], fromView.getMeasuredWidth(), fromView.getMeasuredHeight(),
                position, imageObjList);
